...
export function formQuantity() {
	document.addEventListener("click", function (e) {
		let targetElement = e.target;
		if (targetElement.closest('.quantity-ibl__button')) {
			let value = parseInt(targetElement.closest('.quantity-ibl').querySelector('input').value);
			if (targetElement.classList.contains('quantity-ibl__button_plus')) {
				value++;
			} else {
				--value;
				if (value < 1) value = 1;
			}
			targetElement.closest('.quantity-ibl').querySelector('input').value = value;
		}
	});
}
